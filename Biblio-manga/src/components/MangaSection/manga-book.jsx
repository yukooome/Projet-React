...
// function MangaBook({ showMyList, myMangaList, toggleMangaInList, searchTerm }) {
...
//   // Fonction pour récupérer les mangas de l'API avec Axios
//   const getMangas = async (search = '', page = 1) => {
//     setLoading(true);
//     setError(null);
//     try {
//       const response = await axios.get(`https://api.jikan.moe/v4/manga?q=${search}&limit=10&page=${page}`);
//       if (page === 1) {
//         setMangas(response.data.data); // Réinitialise la liste des mangas si c'est la première page
//       } else {
//         setMangas((prev) => [...prev, ...response.data.data]); // Ajoute plus de mangas
//       }
//       setHasMore(response.data.data.length > 0); // Vérifie s'il reste des mangas à charger
//     } catch (error) {
...
//   // Utilise useEffect pour charger les mangas quand le terme de recherche change
//   useEffect(() => {
//     getMangas(searchTerm);
//   }, [searchTerm]);

//   // Fonction pour charger plus de mangas
//   const loadMoreMangas = () => {
//     setCurrentPage((prev) => prev + 1);
//     getMangas(searchTerm, currentPage + 1);
//   };

//   // Utilise un autre effet pour charger les mangas supplémentaires quand la page change
//   useEffect(() => {
//     if (currentPage > 1) {
//       getMangas(searchTerm, currentPage);
//     }
...
//       {(showMyList ? mangas.filter(manga => myMangaList.includes(manga.mal_id)) : mangas).map((manga) => (
//         <div key={manga.mal_id} className='manga-item'>
//           <img src={manga.images.jpg.large_image_url} alt={manga.title} />
//           <p>{manga.title}</p>
//           <button onClick={() => toggleMangaInList(manga.mal_id)} className={myMangaList.includes(manga.mal_id) ? "remove-button" : "add-button"}>
